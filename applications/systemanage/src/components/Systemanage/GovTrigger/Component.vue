...
<template>
  <div class="container">
    <el-breadcrumb :separator-icon="ArrowRight">
      ...
      <el-breadcrumb-item>{{ $t(`Menu['触发器']`) }}</el-breadcrumb-item>
    </el-breadcrumb>
    <div class="panel">
      <div class="title-panel">
        <div class="info">
          <div class="info-detail">
            <b class="title">{{ $t(`Menu['触发器']`) }}<span class="count">({{ count }})</span></b>
          </div>
          <div class="info-btn-group">
            <el-button type="info" plain class="info-btn" @click="queryGovTasksData(0)">{{ $t(`common['刷新']`) }}</el-button>
            <!-- <el-button type="primary" class="info-btn">创建触发器</el-button> -->
          </div>
        </div>
      </div>
      <div class="mid-panel">
        <el-input v-model="search" class="search-bar" :placeholder="$t(`overview['搜索触发器名称']`)" @change="change" @input="change" :prefix-icon="Search" />
        <div class="mid-group">
          <div class="ver-mid">
    ...
    <div class="list">
      <div class="list-panel">
        <el-table ref="multipleTableRef" :data="data" style="width: 100%" @selection-change="handleSelectionChange">
          ...
          <el-table-column property="name" :label="$t(`overview['触发器']`)" width="150">
          </el-table-column>
          <el-table-column property="desc" :label="$t(`common['描述']`)" />
          <el-table-column property="cat" :label="$t(`common['类型']`)" />
...
const queryGovTasksData = (page: number) => {
  try {
    ...
    findAll('triggers', params).then((res: any) => {
      gostore.reset()
      gostore.sync(res.data)
      data.value = gostore.findAll('triggers')
